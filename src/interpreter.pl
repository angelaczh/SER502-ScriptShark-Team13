...
program_eval_h(if(X,Y,Z), Env, NewEnv, Ret) :-
    ...
    (member((_,Val,Bool), Env); (boolean_expr(_, [Val], []), Bool = Val)),
...
program_eval_h(print(X), Env, NewEnv, Ret) :-
    program_eval_h(X, Env, _, Val),
    (member((_,Val,ValN), Env); (number(_,[Val],[]), ValN = Val); (string(Val), ValN = Val); (boolean_expr(_,[Val],[]), ValN = Val)),
    write(ValN), nl,
    NewEnv = Env.
...
program_eval_h(bool_expr(X), Env, NewEnv, Ret) :-
    program_eval_h(X, Env, _, Ret),
    NewEnv = Env.

program_eval_h(and_expr(X,Y), Env, NewEnv, Ret) :-
    program_eval_h(X, Env, _, Val1),
    (member((_,Val1,Bool1), Env); (boolean_expr(_, [Val1], []), Bool1 = Val1)),

    program_eval_h(Y, Env, _, Val2),
    (member((_,Val2,Bool2), Env); (boolean_expr(_, [Val2], []), Bool2 = Val2)),
...
program_eval_h(eq(X,Y), Env, NewEnv, Ret) :-    
    program_eval_h(X, Env, _, Val1),
    ((member((_,Val1,Val1N), Env)); (number(_,[Val1],[]), Val1N = Val1); (string(Val1), Val1N = Val1); (boolean_expr(_,[Val1],[]), Val1N = Val1)),

    program_eval_h(Y, Env, _, Val2),
    (member((_,Val2,Val2N), Env); (number(_,[Val2],[]), Val2N = Val2); (string(Val2), Val2N = Val2); (boolean_expr(_,[Val2],[]), Val2N = Val2)),
...
program_eval_h(str_expr(X), Env, NewEnv, Ret) :-
    program_eval_h(X, Env, NewEnv, Ret).
